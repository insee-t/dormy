"use client"

import {
  Form,
  FormControl,
  FormField,
  FormItem,
  FormLabel,
  FormMessage,
} from "@/components/ui/form"
// import { oAuthSignIn, signIn } from "../actions"
import { useForm } from "react-hook-form"
import { z } from "zod"
import { Input } from "@/components/ui/input"
import { Button } from "@/components/ui/button"
import { useState } from "react"
import { signInSchema } from "../schemas"
import Link from "next/link"
import { oAuthSignIn, signIn } from "../actions"

export function SignInForm() {
  const [error, setError] = useState<string>()
  const [userType, setUserType] = useState<'user' | 'admin'>('user')
  const form = useForm<z.infer<typeof signInSchema>>({
    defaultValues: {
      email: "",
      password: "",
    },
  })

  async function onSubmit(data: z.infer<typeof signInSchema>) {
    const error = await signIn(data)
    setError(error)
  }

  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        {error && (
          <div className="p-4 text-sm text-destructive bg-destructive/10 rounded-lg border border-destructive/20">
            {error}
          </div>
        )}
        
        {/* User Type Selector */}
        <div className="space-y-3">
          <FormLabel className="text-sm font-medium text-gray-700">ประเภทผู้ใช้</FormLabel>
          <div className="grid grid-cols-2 gap-3">
            <Button
              type="button"
              onClick={() => setUserType('user')}
              className={`h-12 font-medium transition-colors ${
                userType === 'user'
                  ? 'bg-[#01BCB4] text-white hover:bg-[#01a8a0]'
                  : 'bg-gray-100 text-gray-700 hover:bg-gray-200 border border-gray-300'
              }`}
              variant={userType === 'user' ? 'default' : 'outline'}
            >
              ผู้เช่า
            </Button>
            <Button
              type="button"
              onClick={() => setUserType('admin')}
              className={`h-12 font-medium transition-colors ${
                userType === 'admin'
                  ? 'bg-[#01BCB4] text-white hover:bg-[#01a8a0]'
                  : 'bg-gray-100 text-gray-700 hover:bg-gray-200 border border-gray-300'
              }`}
              variant={userType === 'admin' ? 'default' : 'outline'}
            >
              เจ้าของหอพัก
            </Button>
          </div>
        </div>

        {/* Google Sign In Button - Only for Admin */}
        {userType === 'admin' && (
          <div className="space-y-5">
            <Button
              type="button"
              onClick={async () => await oAuthSignIn("google")}
              className="w-full flex items-center justify-center gap-3 py-4 px-6 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors text-gray-700 font-medium"
              variant="outline"
            >
              <svg className="w-5 h-5" viewBox="0 0 48 48">
                <path
                  fill="#EA4335"
                  d="M24 9.5c3.54 0 6.71 1.22 9.21 3.6l6.85-6.85C35.9 2.38 30.47 0 24 0 14.62 0 6.51 5.38 2.56 13.22l7.98 6.19C12.43 13.72 17.74 9.5 24 9.5z"
                />
                <path
                  fill="#4285F4"
                  d="M46.98 24.55c0-1.57-.15-3.09-.38-4.55H24v9.02h12.94c-.58 2.96-2.26 5.48-4.78 7.18l7.73 6c4.51-4.18 7.09-10.36 7.09-17.65z"
                />
                <path
                  fill="#FBBC05"
                  d="M10.53 28.59c-.48-1.45-.76-2.99-.76-4.59s.28-3.14.76-4.59l-7.98-6.19C.92 16.46 0 20.12 0 24c0 3.88.92 7.54 2.56 10.78l7.97-6.19z"
                />
                <path
                  fill="#34A853"
                  d="M24 48c6.48 0 11.93-2.13 15.89-5.81l-7.73-6c-2.15 1.45-4.92 2.3-8.16 2.3-6.26 0-11.57-4.22-13.47-9.91l-7.98 6.19C6.51 42.62 14.62 48 24 48z"
                />
                <path fill="none" d="M0 0h48v48H0z" />
              </svg>
              <span className="text-sm sm:text-base">เข้าสู่ระบบด้วย Google</span>
            </Button>
            
            {/* Divider */}
            <div className="relative">
              <div className="absolute inset-0 flex items-center">
                <span className="w-full border-t border-gray-300" />
              </div>
              <div className="relative flex justify-center text-xs uppercase">
                <span className="bg-white px-3 text-gray-500">หรือ</span>
              </div>
            </div>
          </div>
        )}

        {/* Email Field */}
        <FormField
          control={form.control}
          name="email"
          render={({ field }) => (
            <FormItem className="space-y-3">
              <FormLabel className="text-sm font-medium text-gray-700">อีเมล</FormLabel>
              <FormControl>
                <Input 
                  type="email" 
                  {...field} 
                  className="h-12 text-base px-4"
                  placeholder="กรุณากรอกอีเมล"
                />
              </FormControl>
              <FormMessage className="text-xs" />
            </FormItem>
          )}
        />

        {/* Password Field */}
        <FormField
          control={form.control}
          name="password"
          render={({ field }) => (
            <FormItem className="space-y-3">
              <FormLabel className="text-sm font-medium text-gray-700">รหัสผ่าน</FormLabel>
              <FormControl>
                <Input 
                  type="password" 
                  {...field} 
                  className="h-12 text-base px-4"
                  placeholder="กรุณากรอกรหัสผ่าน"
                />
              </FormControl>
              <FormMessage className="text-xs" />
            </FormItem>
          )}
        />

        {/* Sign In Button */}
        <div className="pt-4">
          <Button
            type="submit"
            className="w-full h-12 bg-[#01BCB4] hover:bg-[#01a8a0] text-white font-medium text-base transition-colors"
          >
            เข้าสู่ระบบ
          </Button>
        </div>

        {/* Sign Up Link */}
        <div className="pt-6 text-center">
          <p className="text-sm text-gray-600">
            <span>ยังไม่มีบัญชี ?</span>
            <span className="ml-1">
              <Link
                href={userType === 'user' ? '/tenant/register' : '/register'}
                className="text-[#01BCB4] hover:text-[#01a8a0] font-medium hover:underline transition-colors"
              >
                สร้างบัญชีที่นี่
              </Link>
            </span>
          </p>
        </div>
      </form>
    </Form>
  )
}
